...
{{--            <h6 class="ml-3">Mostrando {{ $medias_temp->count() }} de {{ $medias_temp->total() }} mídias coletadas</h6>--}}
            @if($term)
                {{ $medias_temp->onEachSide(1)->appends(['term' => $term])->links('vendor.pagination.simple-bootstrap-4') }}
            @else
                {{ $medias_temp->onEachSide(1)->links('vendor.pagination.simple-bootstrap-4') }}
            @endif
