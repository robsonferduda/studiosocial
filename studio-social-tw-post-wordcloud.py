...
sql = 'select id, full_text as text, client_id from media_twitter where deleted_at is null and wordcloud_expression is null'
...
for media in medias:
    ...
    text = "".join(c.lower() for c in media['text'])
    ...
    wordcloud_words = json.dumps(wordcloud_words) 
    
    sql = "update media_twitter SET wordcloud_expression = '"+wordcloud_words+"' WHERE id = "+str(media['id'])+' and client_id='+str(media['client_id'])
    print(sql)
