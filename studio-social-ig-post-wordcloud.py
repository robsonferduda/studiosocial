...
sql = 'select id, caption as text, client_id from medias where deleted_at is null and wordcloud_expression is null'
...
for media in medias:
    ...
    sql = "update medias SET wordcloud_expression = '"+wordcloud_words+"' WHERE id = "+str(media['id'])+' and client_id='+str(media['client_id'])
    print(sql)
