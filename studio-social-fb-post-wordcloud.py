...
sql = 'select id, message as text, client_id from fb_posts where deleted_at is null and wordcloud_expression is null'
...
for media in medias:
    ...
    sql = "update fb_posts SET wordcloud_expression = '"+wordcloud_words+"' WHERE id = "+str(media['id'])+' and client_id='+str(media['client_id'])
    print(sql)
