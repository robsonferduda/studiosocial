...
for client in clients:
    ...
    sql_words_exception = 'select word from words_exception where client_id ='+str(client['id'])
    cur.execute(sql_words_exception)
    ...
    df_words_exception = pd.DataFrame (words_exception)
    removeWords = df_words_exception.to_numpy()

    text = re.sub(r"www\S+", " ", text)
    text = re.sub(r"http\S+", " ", text)
    text = re.sub(r"#([a-zA-Z0-9_]{1,50})", " ", text)

    for word in removeWords.tolist():
        text = re.sub(r'(,|.|^|\s+)'+word[0]+'($|\s+|.|,)', '', text)
